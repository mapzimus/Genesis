...
As created, neither Routine carries MCP connectors, so a fired session would lack GitHub issue tools. That gap is recorded in `READINESS_EVIDENCE.md` and must be closed before enabling. Attaching the GitHub connector to the Routines is a laboratory-infrastructure change, not a business connector authorization, and does not by itself authorize any external business write.
